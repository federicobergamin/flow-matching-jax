"""
File where we train a simple model to learn a velocity filed to go from a single Gaussian 
to a mixture of 8-Gaussian using the original framework proposed by Lipman et al.
"""
...
def main(args):
    ...
    p1_samples = sample_8gaussians(n_training_points)
    ...
    for ep in tqdm(range(epochs), desc="Training"):
        ep_loss = 0
        ep_num_elem = 0
        for batch_p1 in train_loader_p1:

            key_t, key_p0, key_eps, prng_key = split_key(prng_key, num=4)
            t = jax.random.uniform(key_t, shape=(batch_p1.shape[0], 1))

            # right now we have independent couplings
            # for p0 I have to sample from a standard Gaussian distribution
            batch_p0 = sample_gaussian(batch_p1.shape[0], dimension=2, key=key_p0)
    ...
    x0_samples = sample_gaussian(5000, dimension=2, key=key_samples)
    t = jnp.zeros((5000, 1))
