...
I have taken from https://github.com/yang-song/score_sde/blob/0acb9e0ea3b8cccd935068cd9c657318fbc6ce4c/models/ddpm.py
"""
...
from cfm_jax.models.unet_v2 import layers, normalization
...
if __name__ == "__main__":
    ...
    # now I have to init the model 
    prng_key = jax.random.PRNGKey(0)
    batch_size = 4
    key_sampling_x0, key_sampling_t, init_rng, prng_key = jax.random.split(prng_key, 4)
    input_shape = (batch_size, 28, 28, 1)

    x = jax.random.normal(key_sampling_x0, input_shape)
    time = jax.random.uniform(key_sampling_t, shape=(batch_size,))

    params_dict = model.init(init_rng, x, time, train=False)
    params_vec, unflatten = flatten_util.ravel_pytree(params_dict)
    n_params = len(params_vec)
    print(f"Number of parameters: {n_params}")
    # x = jnp.ones((1, 28, 28, 1))
    # labels = jnp.ones((1,)) * 500
    # out = model.apply(variables={}, x=x, labels=labels)
    # print(out.shape)
    # now I can try to get the output of the model
    dropout_key, prng_key = jax.random.split(prng_key)
    out = model.apply({"params": params_dict["params"]}, x, time, train=True, rngs={'dropout': dropout_key})
    print(out.shape)